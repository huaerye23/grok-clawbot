...
3. **立刻 `wechat_start_monitor`**（独立 OS 进程，空闲 **0 token**）。不要为了省 token 让 Bot 睡，也不要用「每 5 分钟喊模型」当主保活。Webhook 只是主机把会话卸掉时的退路。有 Rust 产物用 Rust（`npm run monitor:build`），否则 `node server/index.js --monitor`。
